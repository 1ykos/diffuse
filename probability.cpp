...
int main(int argc, char *argv[])
{
  ...
  const double energy = pFF->Energy();
  cerr << "Molecule energy= " << energy << " " << pFF->GetUnit() << endl;
  cerr << energy*1000/(R*298) << endl;
  //cerr << "read molecule" << endl;
  //cerr << "Molecule has " << molecule.NumHvyAtoms() << " heavy atoms" << endl;
  int16_t* data = new int16_t[dim3];
  if (argc<2){
    cerr << "Please supply file with diffraction data" << endl;
    return 127;
  }
  ifstream datafile(argv[1]);
  //cerr << "reading in diffraction data" << endl;
  datafile.read(reinterpret_cast<char*>(data),2*dim3);
  //cerr << "read in diffraction data" << endl;
  //cerr << "zero frequency of data is:" << data[0] << endl;
    ...
    for (size_t y = 0; y!=dim; ++y){
      for (size_t x = 0; x!=dim; ++x){
        ...
        const double l = (out[n][0]*out[n][0]+out[n][1]*out[n][1])
                         /total*num_photons;
        //l**k/gamma(k+1)*exp(-l)
        //log(l**k/gamma(k+1)*exp(-l)
        //k*log(l)-lgamma(k+1)-l
        //lgamma(k+1)+l-k*log(l)
        const double k = data[n];
        score += lgamma(k+1)+l-k*log(l);
      }
    }
  }
  cout << "-LLG= " << score << endl;
